# ansible isolation composition probe v1
"""Hostile fixture intended to be committed, snapshotted, then sandbox-executed."""
import json
import os
...
def baseline(secret_path: str) -> int:
    report = {}
    report["input_read"] = Path("/input/ansible_sandbox_probe.py").read_bytes().startswith(
        b"# ansible isolation composition probe v1\n")
    try:
    ...
    report["credential_env_absent"] = all(name not in os.environ for name in (
    ...
    report["result_write"] = Path("/output/allowed.txt").read_text(encoding="ascii") == "allowed\n"
    child = subprocess.run(
        [sys.executable, "-I", "-S", "-B", "-c",
         "from pathlib import Path; Path('/output/child.txt').write_text('child\\n', encoding='ascii')"],
        stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
        timeout=2, check=False)
    report["child_confined_write"] = (child.returncode == 0
        and Path("/output/child.txt").read_text(encoding="ascii") == "child\n")
